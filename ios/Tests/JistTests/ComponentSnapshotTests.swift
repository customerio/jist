...
final class ComponentSnapshotTests: XCTestCase {
    ...
    func testAllComponentFixtures() throws {
        ...
        // JSONSerialization slices the fixture document; everything handed to
        // the renderer is parsed by jist-core (Rust), same as production.
        for fixtureURL in fixtureFiles {
            ...
            for (caseName, caseValue) in fixtureJSON.sorted(by: { $0.key < $1.key }) {
                ...
                let template = try parseTemplateJson(json: String(decoding: wrapperData, as: UTF8.self))

                // Parse data through jist-core
                ...
                if let dataJSON = caseDict["data"] {
                    ...
                    caseData = try parseDataJson(json: String(decoding: dataBytes, as: UTF8.self))
                }
                ...
                if let themeJSON = caseDict["theme"] as? [String: Any], !themeJSON.isEmpty {
                    ...
                    caseTheme = try parseDataJson(json: String(decoding: themeBytes, as: UTF8.self))
                }
